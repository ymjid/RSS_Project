...
<title>Website's title  - Archives</title>
<meta charset="utf-8" />
...
echo '<div id="fluxes">';	
 	...
 	while ($donnees2 = $resultat2->fetch()) {
 		...
 		$numflux=$donnees2['ID'];
 		$test = $bdd->query('SELECT * FROM rssarticle WHERE fluxID='.$numflux.' && state="0" && statearticle!="desactivated" && numuser="'.$_SESSION['userid'].'" ORDER BY articledate DESC');
   			if ( 0 != $test->fetchColumn()){
   				$fluxnb++;
 				echo '<div id=flux_'.$numflux.'>';
 		 		echo '<p class="title">'.$domain.'</p>';
 				echo '<br><br>';
 				$resultat3 = $bdd->query('SELECT * FROM rssarticle WHERE fluxID='.$numflux.' && state="0" && statearticle!="desactivated" && numuser="'.$_SESSION['userid'].'" ORDER BY articledate DESC');
 				while ($donnees3 = $resultat3->fetch()) {
 					...
 					$date = $donnees3['articledate'];
 						$page = "";
 						$page .="<ul>";
 						$page .= "<li class=article name=".$articleID."><a href=".$link." target=\"_blank\" >".$title."</a>";	
						if($date!="")
						{
